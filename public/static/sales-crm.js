    ...
    <div class="bg-white rounded-xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto" onclick="event.stopPropagation()">
      ...
      <form onsubmit="submitNewMeeting(event, ${prospectId})" class="p-6 space-y-4">
        <div class="grid grid-cols-2 gap-4">
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">商談タイプ <span class="text-red-500">*</span></label>
        ...
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-2">商談結果</label>
          <select id="meeting_outcome" class="w-full px-3 py-2 border border-gray-300 rounded-lg">
            <option value="">選択してください</option>
            <option value="successful">成功</option>
            <option value="pending">保留中</option>
            <option value="follow_up_needed">要フォローアップ</option>
            <option value="unsuccessful">不成功</option>
          </select>
        </div>
        <div>
...
async function submitNewMeeting(event, prospectId) {
  ...
  const formData = {
    ...
    meeting_outcome: document.getElementById('meeting_outcome').value || null
  };
